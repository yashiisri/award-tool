...
from services.research.entity_classifier import is_person, NOT_NAMES, GENERIC_WORDS, ROLE_WORDS
...
# Common single-word locations that otherwise pass every organization-name
# check (capitalized, no stray lowercase words) — a phrase like "...speaking
# in California..." must not end up labeled as someone's organization.
_LOCATION_WORDS = {
    "california", "texas", "florida", "washington", "york", "london",
    "delhi", "mumbai", "bangalore", "bengaluru", "chennai", "hyderabad",
    "india", "america", "europe", "asia", "china", "japan", "africa",
    "australia", "canada", "seattle", "boston", "chicago", "redmond",
}
...
def _extract_org(text: str) -> str:
    for m in ORG_RE.finditer(text[:500]):
        ...
        if candidate.lower() in _LOCATION_WORDS:
            continue
        # Reject only when the candidate is made up entirely of role words and
        # connectors (e.g. "Director at Director") — a role word merely
        # prefixing a real org ("CEO of Acme Robotics") should still yield
        # the org name, not get thrown out wholesale.
        non_connector_words = [w for w in candidate.lower().split() if w not in _ORG_CONNECTORS]
        if non_connector_words and all(w in ROLE_WORDS for w in non_connector_words):
            continue
...
def extract_candidates(results: list[SearchResult], role_keywords: list[str] | None = None) -> list[dict]:
    ...
    for r in results:
        ...
        if m and _valid_name(m.group(1)) and is_person(m.group(1)):
            found_names.append(m.group(1))

        for match in NAME_RE.finditer(text):
            candidate = match.group(1)
            # entity_classifier is the authoritative PERSON/COMPANY/ORGANIZATION
            # gate — a name-shaped string that's actually a company or org
            # (e.g. "Morgan Stanley") never becomes a candidate, regardless of
            # how plausible it looks structurally.
            if candidate in found_names:
                continue
            if _valid_name(candidate) and is_person(candidate):
