...
export default function HJVoting() {
  ...
  const fetchData = async () => {
    try {
      ...
      const map = {}; voteRes.data.forEach(v => { map[v.nominee_id] = v }); setMyVotes(map)
    } catch (e) {}
  }
  ...
  const handleVote = async (nomineeId, rank) => {
    ...
    try {
      ...
      await fetchData(); setSelectedNominee(null)
    } catch (e) { alert(e.response?.data?.detail || 'Voting failed') }
    finally { setLoading(false) }
  }

  return (
    <div className="p-8">
      <PageHeader icon={Vote} title="Your Vote" subtitle="Cast your votes for nominees" accent="#7F3F98" light="#F5EEF8" />
      ...
      {selectedAward && votingOpen && (
        <div className="mb-6 flex items-center gap-3 p-4 bg-green-50 border border-green-200 rounded-xl">
          ...
          <p className="font-semibold text-green-800 text-sm">Voting is open — {Object.keys(myVotes).length}/{nominees.length} votes cast</p>
        </div>
      )}
      ...
      <div className={`grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 ${!votingOpen ? 'opacity-50 pointer-events-none select-none' : ''}`}>
        {nominees.map(nom => {
          ...
          return (
            <div key={nom.id} onClick={() => votingOpen && !myVote && setSelectedNominee(nom)}
              className={`bg-white border rounded-2xl overflow-hidden transition-all ${myVote ? 'border-green-200' : votingOpen ? 'border-gray-100 hover:shadow-xl hover:border-[#7F3F98]/20 cursor-pointer hover:-translate-y-0.5' : 'border-gray-100'}`}>
              <div className="h-28 bg-gradient-to-br from-[#F5EEF8] to-[#ead5f5] flex items-center justify-center relative">
                ...
                {myVote && rankInfo && (
                  <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                    <div className="text-2xl font-black text-white">{rankInfo.label}</div>
                  </div>
                )}
              </div>
              <div className="p-5">
                ...
                {myVote ? (
                  ...
                  <div className="py-2 px-3 bg-[#F5EEF8] text-[#7F3F98] rounded-xl text-xs font-semibold text-center">Tap to vote</div>
                ) : (
      ...
      {selectedNominee && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={() => setSelectedNominee(null)}>
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm" onClick={e => e.stopPropagation()}>
            <div className="bg-gradient-to-br from-[#7F3F98] to-[#5B2D6E] p-6 rounded-t-3xl relative">
              <button onClick={() => setSelectedNominee(null)} className="absolute top-4 right-4 w-8 h-8 bg-white/20 hover:bg-white/30 rounded-lg flex items-center justify-center"><X className="w-4 h-4 text-white" /></button>
