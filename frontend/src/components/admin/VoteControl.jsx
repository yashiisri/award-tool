...
function Toggle({ active, onClick }) {
  return (
    <button type="button" onClick={onClick} style={{
      position: 'relative', width: 38, height: 21, borderRadius: 99, flexShrink: 0,
      ...
      <div style={{ padding: '32px 36px' }}>
        ...
        <div style={{ marginBottom: 26 }}>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: 13, marginTop: 4 }}>
            Control when jury and head jury can nominate and vote, per award category
          </p>
        </div>
        ...
        {awards.length === 0 ? (
          <div style={{ padding: '80px 24px', background: '#fff', borderRadius: 12, border: '1px solid var(--border-light)', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <Settings size={36} color="var(--gold)" strokeWidth={1.5} style={{ marginBottom: 18 }} />
            <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 19, color: 'var(--kpmg-navy)', marginBottom: 8 }}>No awards to configure</p>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {awards.map(award => {
              ...
              return (
                <div key={award.id} style={{ background: '#fff', borderRadius: 10, border: '1px solid var(--border-light)', overflow: 'hidden' }}>
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', padding: '18px 22px', borderBottom: '1px solid var(--border-light)' }}>
                    ...
                    <button onClick={() => save(award.id)} disabled={saving[award.id]} style={{
                      padding: '8px 18px', borderRadius: 6, background: saving[award.id] ? '#9BA8B5' : 'var(--kpmg-blue)', color: '#fff',
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '12px 22px', borderTop: '1px solid var(--border-light)', background: '#FAFBFD' }}>
                    <span style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 3, fontSize: 11, fontWeight: 600, background: ctrl.voting_enabled ? '#F0FDF4' : '#F4F5F7', color: ctrl.voting_enabled ? '#15803D' : 'var(--text-muted)' }}>
                    ...
                    <span style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px', borderRadius: 3, fontSize: 11, fontWeight: 600, background: ctrl.nomination_enabled ? '#EEF3FF' : '#F4F5F7', color: ctrl.nomination_enabled ? 'var(--kpmg-blue)' : 'var(--text-muted)' }}>
